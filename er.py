...
parser.add_argument('-d', '--dataset', type=str, default='permuted_mnist',
                    choices=['permuted_mnist'])
...
# ------ MIR -----#
parser.add_argument('-m', '--method', type=str, default='no_rehearsal',
                    choices=['no_rehearsal', 'rand_replay', 'mir_replay'])
...
for run in range(args.n_runs):
    ...
    for task, tr_loader in enumerate(train_loader):
        ...
        for i, (data, target) in enumerate(tr_loader):
            if args.unit_test and i > 10:
                break
            if sample_amt > args.samples_per_task > 0:
                break
